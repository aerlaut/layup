...
5. If the user mentions that he/she would like to address the issue in separate sessions, or ask to save the plan, save the corresponding task details in each file in separate Markdown files under `.pi/todo/`. Make sure to include a brief section on the motiviation of the task, before the task details to provide context. Create a git commit with a short descriptive message to save the plans ensure that they're not deleted by mistake.