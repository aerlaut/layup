...
- **Navigation stack out of sync in "no focus" state**: When no group is focused, `currentDiagram` still points to whatever was last focused. Node placement or edge creation in this state would silently target that stale diagram, confusing users. → Mitigation: Disable node placement and edge creation in "no focus" mode. In `handlePaneClick`, skip the pending-node-type placement when `focusedParentNodeId` is null. In `handleConnect`, reject connections when no group is focused. The palette selection (pending node type) is cleared when entering no-focus mode to provide visual feedback that placement is unavailable. This is low complexity — a single guard check in each handler — and prevents silent misplacement.
