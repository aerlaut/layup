...
- [x] 3.2 Verify double-clicking background navigates up one level, and does nothing at root

## 4. Node Overlap Prevention

- [x] 4.1 Create an overlap detection utility function that checks if two node bounding boxes intersect, using node position and dimensions
- [x] 4.2 Create a `resolveOverlaps` function that pushes overlapped nodes away along the axis of least overlap, with iterative resolution (max 10 passes)
- [x] 4.3 Integrate `resolveOverlaps` into `handleNodeDragStop` — after updating the moved node's position, run overlap resolution on sibling nodes in the same group
- [x] 4.4 Verify dragging a node onto another pushes the overlapped node away, including chain displacement

## 5. Boundary Overlap Prevention

- [x] 5.1 Create a `resolveBoundaryOverlaps` function that detects overlapping boundary bounding boxes and shifts the smaller group's nodes by a uniform offset
- [x] 5.2 Integrate boundary overlap check after node position changes (drag stop, node add, node overlap resolution)
- [x] 5.3 Verify moving/adding nodes that cause boundary expansion triggers sibling group repositioning while preserving internal layout
